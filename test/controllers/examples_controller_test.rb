...
class ExamplesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create example" do
    assert_difference('Example.count') do
      post examples_url, params: { example: { bool: @example.bool, description: @example.description, name: @example.name } }
    end
  ...
  test "should update example" do
    patch example_url(@example), params: { example: { bool: @example.bool, description: @example.description, name: @example.name } }
